...
// Player Component
export const Player = forwardRef(({ disableForward = false, gameStarted = true, position = [0, 4, -10] }, ref) => {
    ...
    const playerMove = ({ forward, backward, left, right, rotationYVelocity, newVelocity, }) => {
        if (!rigidBodyRef.current)
            return;
        const velocity = rigidBodyRef.current.linvel();
        const currentRigidBodyRotation = rigidBodyRef.current.rotation();
        const currentQ = new THREE.Quaternion(currentRigidBodyRotation.x, currentRigidBodyRotation.y, currentRigidBodyRotation.z, currentRigidBodyRotation.w);
        if (rotationYVelocity !== 0) {
            currentQ.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, rotationYVelocity, 0, 'YXZ')));
            rigidBodyRef.current.setRotation(currentQ, true);
        }
        if (newVelocity) {
            rigidBodyRef.current.setLinvel({ x: newVelocity.x, y: velocity.y, z: newVelocity.z }, true);
            return;
        }
        // VR入力に基づく前後左右
    ...
    useFrame((state) => {
        ...
        if (useFreeLook) {
            ...
            if (direction.lengthSq() > 0) {
                ...
                rigidBodyRef.current.setLinvel({ x: 0, y: velocity.y, z: 0 }, // 左右前後の動きを止める
                true);
            }
            ...
            const playerQuaternion = new THREE.Quaternion(playerWorldRotation.x, playerWorldRotation.y, playerWorldRotation.z, playerWorldRotation.w);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(PointerLockControls, { makeDefault: true }), _jsxs(RigidBody, { ref: rigidBodyRef, colliders: false, type: "dynamic", mass: 1, position: position, rotation: [0, -0.28, 0], enabledRotations: [false, false, false], collisionGroups: interactionGroups([0], [0]), children: [_jsx(CapsuleCollider, { args: [0.7, 0.35] }), _jsx(IfInSessionMode, { allow: ['immersive-vr'], children: _jsx(VRController, { playerJump: playerJump, playerMove: playerMove }) })] })] }));
});
